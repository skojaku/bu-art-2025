...
preprocessed_paper_concept_table_file = snakemake.output[
    "preprocessed_paper_concept_table_file"
]
preprocessed_concept_table_file = snakemake.output["preprocessed_concept_table_file"]
preprocessed_author_paper_table_file = snakemake.output[
    "preprocessed_author_paper_table_file"
]
...
unique_paper_table = (
    pd.concat([unique_title_papers, filtered_duplicates])
    .sort_values("paper_id")
    .reset_index(drop=True)
)

# Verify no duplicates remain
assert len(unique_paper_table["title"].unique()) == len(
    unique_paper_table
), "Duplicate titles found in unique_paper_table"


# Create mapping from old to new paper IDs
paper_id_mapping = dict(
    zip(unique_paper_table["paper_id"], range(len(unique_paper_table)))
)
